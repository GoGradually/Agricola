...
class StackResources(Command):
    def execute(self):
        for i in game_status_repository.game_status_repository.game_status.round_card_put:
            i = None
        for i in game_status_repository.game_status_repository.game_status.basic_card_put:
            i = None
        game_status = game_status_repository.game_status_repository.game_status
        for i in range(game_status.now_round + 1):
            ...
            if game_status.round_card_order[i] == RoundBehaviorType.STONE_4.value:
                game_status.set_round_resource(i, game_status.round_resource[i] + 1)
            if game_status.round_card_order[i] == RoundBehaviorType.VEGETABLE.value:
                game_status.set_round_resource(i, 1)
