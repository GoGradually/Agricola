...
class Oven1(MainFacilityInterface):
    def __init__(self, input_behavior):
        ...
        self.main_card_type = MainFacilityType.OVEN1
    ...
    def canUse(self):
        return isinstance(self.input_behavior,
    ...
    def doChange(self, resource_type, resource_value, pos): # 화로 기능 아무떄나 처리하는 함수 빵굽기는 execute()
        ...
        if resource_type == 0 and self.player_data.resource.vegetable >= resource_value:
            ...
            if DumpAnimal(AnimalType.SHEEP, pos):
                self.player_data.resource.food += resource_value * 2
                self.log_text = f"양 {resource_value}마리를 음식 {resource_value * 2}개로 요리했습니다"
                return True
            return False
            ...
            if DumpAnimal(AnimalType.PIG, pos):
                self.player_data.resource.food += resource_value * 2
                self.log_text = f"돼지 {resource_value}마리를 음식 {resource_value * 2}개로 요리했습니다"
                return True
            return False
            ...
            if DumpAnimal(AnimalType.COW, pos):
                ...
                self.log_text = f"소 {resource_value}마리를 음식 {resource_value * 3}개로 요리했습니다"
                return True
            return False
    ...
    def purchase(self):
        ...
        self.game_status.main_facility_status[1] = self.game_status.now_turn_player
        self.player_data.resource.set_dirt(self.player_data.resource.dirt - 2)
    ...
    def canPurchase(self):
        return self.player_data.resource.dirt >= 2 and  self.game_status.main_facility_status[1] == -1
