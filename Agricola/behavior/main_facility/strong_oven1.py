...
class StrongOven1(MainFacilityInterface):
    ...
    def purchase(self):
        ...
        self.get_game_status().main_facility_status[3] = self.get_game_status().now_turn_player
    ...
    def canPurchase(self):
        chk_oven = any(mainCard.main_card_type in (MainFacilityType.OVEN1, MainFacilityType.OVEN2) for mainCard in
                       self.player_data.card.putMainCard)
        return (chk_oven or self.player_data.resource.dirt >= 4) and self.get_game_status().main_facility_status[3] == -1
