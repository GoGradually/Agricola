...
class Theater(BehaviorInterface):
    def __init__(self):
        self.log_text = ""
    ...
    def execute(self):
        self.player_resource.set_food(
            self.player_resource.food + self.get_game_status().basic_resource[BasicBehaviorType.THEATER.value])
        self.log_text = f"음식 {self.get_game_status().basic_resource[BasicBehaviorType.THEATER.value]}개를 획득하였습니다."
        self.get_game_status().set_basic_resource(BasicBehaviorType.THEATER.value, 0)
        return [UseWorker]
