...
class DailyLabor(BehaviorInterface):
    ...
    def execute(self):
        self.player_resource.set_food(
            self.player_resource.food + self.get_game_status().basic_resource[BasicBehaviorType.DAY_LABOR.value])
        self.log_text = f"음식 {self.get_game_status().basic_resource[BasicBehaviorType.DAY_LABOR.value]}개를 획득하였습니다."
        self.get_game_status().set_basic_resource(BasicBehaviorType.DIRT1.value, 2)
        return [UseWorker]
