...
class Dirt2(BehaviorInterface):
    ...
    def execute(self):
        self.player_resource.set_dirt(
            self.player_resource.dirt + self.get_game_status().basic_resource[BasicBehaviorType.DIRT2.value])
        self.log_text = f"흙 {self.get_game_status().basic_resource[BasicBehaviorType.DIRT2.value]}개를 획득하였습니다."
        self.get_game_status().set_basic_resource(BasicBehaviorType.DIRT2.value, 0)
        return [UseWorker]
